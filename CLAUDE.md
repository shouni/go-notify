...
- `notifier.go` — the unexported `notifier` holds the requester and the webhook URL, nothing else. `NewNotifier` holds the disabled-vs-error policy above; `Notify` implements `notify.Notifier`. An empty `Message.Title` is an error, not a cue to guess: deriving a heading from the body's first line was a feature nothing used, and `Pipeline.send` already rejects an empty title, so the library said no through two separate paths. A send failure comes back as the unexported `sendError`, whose `Error()` replaces the webhook URL with `<webhook URL>` and whose `Unwrap()` returns the cause untouched. The URL *is* the credential for an Incoming Webhook, and both `net/http` (`*url.Error`) and httpkit (`HTTPリクエスト失敗 (URL: …)`) put the destination into the error text, which every service then hands to `slog.Error` as-is — so the redaction has to happen here, on the only path that knows the URL. Don't turn this into a `fmt.Errorf` again. `TestNotifyRedactsWebhookURLInSendError` pins it.
- `payload.go` — the wire shape of the Incoming Webhook payload (`webhookMessage`, `attachment`, `block`, `textObject`) and the four block constructors. **This used to come from `slack-go/slack`, and it was the only thing that did**: no API client, no Socket Mode, no events — five JSON-tagged structs and their constructors, at the cost of `slack-go` plus `gorilla/websocket` in the module graph of every service that notifies (and in their Dependabot and govulncheck queues). The JSON is byte-identical to what `slack-go` produced except for `"replace_original":false,"delete_original":false`, which it emitted without `omitempty` and which mean nothing on a webhook. `TestPayloadWireShape` pins the exact JSON for all four levels; the black-box tests decode the payload through JSON (`sentMessage`) rather than through internal types, so a future type change cannot pass while the wire form drifts. Don't re-import `slack-go` for a convenience type — `ap-mcp-slack` still uses it because it calls the Web API, which is a different job.
- `blocks.go` — Markdown→mrkdwn conversion, escaping, Block Kit assembly, and the size handling for both blocks: the body is **split** into section blocks of at most `maxSectionLength` (2900, under Slack's 3000 limit) at line boundaries (`splitSectionText`), and the heading is truncated at `maxHeaderLength` (150, which *is* Slack's limit). The heading limit is not cosmetic — exceeding it returns `invalid_blocks` and loses the whole notification, so every release before this one could drop a message outright for a long title. `TestNotifyTruncatesLongTitle` pins it.
